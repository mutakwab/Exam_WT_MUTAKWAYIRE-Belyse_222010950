...
<head>
  ...
  <title>course_feedback entity Page</title>
  <style>
    /* Normal link */
    a {
      padding: 10px;
...
<section>
  <center>
      <center><h1><u>Course Feedback Form</u></h1></center>
    <form method="post" action="process_feedback.php" onsubmit="return confirmInsert();">
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO course_feedback(FeedbackID, CourseID, UserID, Feedback, FeedbackDate) VALUES (?, ?, ?, ?, ?)");
    $stmt->bind_param("sssss", $feedbackID, $courseID, $userID, $feedback, $feedbackDate);
...
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Detail Course Feedback</title>
    <style>
        table {
            width: 100%;
...
<body>
     <center><h2>Table of Course Feedback</h2></center>
    <table border="5">
        <tr>
            <th>Feedback ID</th>
            <th>Course ID</th>
        ...
        // Prepare SQL query to retrieve all course feedback
        $sql = "SELECT * FROM course_feedback";
        $result = $connection->query($sql);

        // Check if there is any course feedback
        if ($result->num_rows > 0) {
            // Output data for each row
            while ($row = $result->fetch_assoc()) {
                $feedbackID = $row['FeedbackID']; // Fetch the feedback ID
                echo "<tr>
                    ...
                    <td><a style='padding:4px' href='delete_course_feedback.php?FeedbackID=$feedbackID'>Delete</a></td> 
                    <td><a style='padding:4px' href='update_course_feedback.php?FeedbackID=$feedbackID'>Update</a></td> 
                </tr>";
            }
        } else {
