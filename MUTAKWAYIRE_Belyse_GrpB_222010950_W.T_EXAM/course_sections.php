...
<head>
  ...
  <title>course sections entity Page</title>
  <style>
    /* Normal link */
    a {
      padding: 10px;
...
<section>
  <center>
  <h1><u> Course Section Form </u></h1>
<form method="post" onsubmit="return confirmInsert();">
   <!-- SectionID CourseID Title Description -->
    <label for="sectionId">Section ID:</label>
    <input type="number" id="sectionId" name="sectionId"><br><br>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO course_sections(SectionID, CourseID, Title, Description) VALUES (?, ?, ?, ?)");
    $stmt->bind_param("ssss", $sectionId, $courseId, $title, $description);
...
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Detail information of Course Sections</title>
    <style>
        table {
            width: 100%;
...
<body>
    <center><h2>Table of Course Sections</h2></center>
    <table border="5">
        <tr>
            <th>Section ID</th>
            <th>Course ID</th>
      ...
      include('db_connection.php');
        ...
        // Prepare SQL query to retrieve all course sections
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                $sectionId = $row['SectionID']; // Fetch the SectionID
                echo "<tr>
                    <td>" . $row['SectionID'] . "</td>
                    <td>" . $row['CourseID'] . "</td>
                    <td>" . $row['Title'] . "</td>
                    <td>" . $row['Description'] . "</td>
                    <td><a style='padding:4px' href='delete_course_sections.php?SectionID=$sectionId'>Delete</a></td> 
                    <td><a style='padding:4px' href='update_course_sections.php?SectionID=$sectionId'>Update</a></td> 
                </tr>";
            }
        } else {
