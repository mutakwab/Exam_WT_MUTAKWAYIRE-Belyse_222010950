...
<head>
  ...
  <title>course_materials Page</title>
  <style>
    /* Normal link */
    a {
      padding: 10px;
...
<section>
  <center>
  <center><h1><u>Course Materials Form</u></h1></center>
    <form method="post" action="process_material.php" onsubmit="return confirmInsert();">
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $stmt = $connection->prepare("INSERT INTO course_materials(MaterialID, SectionID, Type, Link) VALUES (?, ?, ?, ?)");
        $stmt->bind_param("ssss", $materialID, $sectionID, $type, $link);
...
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Detail Course Materials</title>
    <style>
        table {
            width: 100%;
...
<body>
    ...
    <table border="5">
        ...
        // Prepare SQL query to retrieve all course materials
        $sql = "SELECT * FROM course_materials";
        $result = $connection->query($sql);

        // Check if there is any course material
        if ($result->num_rows > 0) {
            // Output data for each row
            while ($row = $result->fetch_assoc()) {
                $materialID = $row['MaterialID']; // Fetch the material ID
                echo "<tr>
                    <td>" . $row['MaterialID'] . "</td>
                    <td>" . $row['SectionID'] . "</td>
                    <td>" . $row['Type'] . "</td>
                    <td>" . $row['Link'] . "</td>
                    <td><a style='padding:4px' href='delete_course_materials.php?MaterialID=$materialID'>Delete</a></td> 
                    <td><a style='padding:4px' href='update_course_material.php?MaterialID=$materialID'>Update</a></td> 
                </tr>";
            }
        } else {
